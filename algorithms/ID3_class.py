...
class ID3_class:
    ...
    # def fit(self, df, target_attribute_name, features):
    def fit(self, X, y):
    ...
    def predict(self, query, tree=None, default=1):
        if tree is None:
            tree = self.tree
    ...
    def get_accuracy(self, X, y, label):
        target_attribute_name = y.name
        df = pd.concat([X, pd.DataFrame({target_attribute_name: y})], axis=1)
        df["classification"] = df.apply(lambda row: self.predict(row), axis=1)
