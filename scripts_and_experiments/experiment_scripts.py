...
from .datasets_manager import get_dataset_corona, get_dataset_divorce, get_dataset_glass, get_dataset_loan_approval
...
def cross_validation_score(X, y, model, n_splits=5):
    ...
    avg_acc_score = round(np.mean(acc_scores), 2)
    avg_f1_score = round(np.mean(f1_scores), 2)
    return avg_acc_score, avg_f1_score
